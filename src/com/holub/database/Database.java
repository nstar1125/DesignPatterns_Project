...
import com.holub.TableExtensions;
 ...
 */
        ...
        public Object get(Object key) {
            ...
            try {
                ...
                if (desiredTable == null) {
                    for (TableExtensions extensions : TableExtensions.values()) {
                        String filename = tableName + "." + extensions.name();
                        if (new File(location,  filename).exists()) {
                            desiredTable = TableFactory.load(filename, location);
    ...
    private void apply(Table current) throws IOException{
        for (TableExtensions extensions : TableExtensions.values()) {
            String filename = current.name() + "." + extensions.name();
            if (new File(location, filename).exists()) {
                Writer out = new FileWriter(new File(location, filename));
                export(extensions, current, out);
                out.close();
                break;
            }
        }
    ...
    private void export(TableExtensions extensions, Table table, Writer out) throws IOException{
        switch (extensions) {
            case csv:
